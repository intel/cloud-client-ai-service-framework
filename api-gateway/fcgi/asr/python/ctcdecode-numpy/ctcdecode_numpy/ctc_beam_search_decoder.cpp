...
static std::vector<std::string> vocab_utf8_1; 
...
void CtcDecoderState::init(
  ...
  if (alphabet.empty()) {
     ...
     for (size_t i = 0; i < 255; ++i) {
        ...
        vocab_utf8_1.push_back(val);
     }
     std::string val(1, 255);
     vocab_utf8_1.push_back(val);

     space_idx_ = ' ' - 1;
     alphabet_ = vocab_utf8_1;
  } else {
     alphabet_ = alphabet;
     // assign space id
...
void CtcDecoderState::append(
  ...
  for (size_t time_step = 0; time_step < probs_frame_num; ++time_step) {
    ...
    std::vector<std::pair<size_t, float>> log_prob_idx =  // create pair (index, prob) for one frame, from big to small
    ...
    for (size_t index = 0; index < alphabet_.size(); index++) {
      auto c = log_prob_idx[index].first; // index 
      auto log_prob_c = log_prob_idx[index].second; // log prob

      for (size_t i = 0; i < candidates_.size() && i < beam_size_; ++i) {
        auto prefix = candidates_[i]; //trei
        ...
        if (c == size_t(prefix->character)) {
          prefix->log_prob_nb_cur = log_sum_exp(//log_prob_nb_cur means none blank prob
              prefix->log_prob_nb_cur, log_prob_c + prefix->log_prob_nb_prev);
        }
        // get/create new prefix; returns null if the new prefix was pruned by a dictionary
        auto prefix_new = prefix->get_path_trie(c, time_step + next_timestep_, log_prob_c); //get next timestep
        ...
        if (prefix_new != nullptr) {
          ...
          if (lm_scorer_ != nullptr) {
            ...
            if (lm_scorer_->is_scoring_boundary(prefix_to_score, c)) {
	      float score = 0.0;
      ...
      }  // end of loop over prefix , candidate size
    }    // end of loop over vocabulary, loop over char
...
void CtcDecoderState::finalize() {
  ...
  if (lm_scorer_ != nullptr) {
    for (size_t i = 0; i < beam_size_ && i < candidates_.size(); ++i) {
      ...
      PathTrie* prefix_boundary = lm_scorer_->is_character_based() ? prefix : prefix->parent;
      if (prefix_boundary && !lm_scorer_->is_scoring_boundary(prefix_boundary, prefix->character)) {
...
std::vector<std::pair<float, Output>> CtcDecoderState::decode(
  ...
  size_t num_candidates = std::min(candidates_.size(), beam_size_);
  std::sort(candidates_.begin(), candidates_.begin() + num_candidates, prefix_compare); //compare score;
...
ctc_beam_search_decoder_batch(
    ...
    const std::vector<size_t>& probs_frame_nums,
