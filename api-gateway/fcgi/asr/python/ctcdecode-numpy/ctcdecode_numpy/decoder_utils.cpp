...
std::vector<std::pair<size_t, float>> get_pruned_log_probs(
  ...
  if (log_cutoff_prob < 0.0 || cutoff_top_n < cutoff_len) {
    std::sort(
        prob_idx.begin(), prob_idx.end(), pair_comp_second_rev<int, float>); // sort from big --> small
...
std::vector<std::string> split_utf8_str(const std::string &str) {
  ...
  for (char c : str) {
    if ((c & 0xc0) != 0x80)  //codepoint_boundary
    {
      if (!out_str.empty()) {
...
bool add_word_to_dictionary(
    ...
    auto characters = add_space ? split_utf8_str(word) : split_into_bytes(word);
