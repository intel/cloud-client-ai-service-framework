...
PathTrie::PathTrie() {
  log_prob_b_prev = -NUM_FLT_INF;  //MAX
...
//new_char is int, means label or index
PathTrie* PathTrie::get_path_trie(int new_char, int new_timestep, float cur_log_prob_c, bool reset) {
  ...
  if (child != children_.end()) {
	...
	new_path->log_prob_c = cur_log_prob_c;
      ...
      PathTrie* new_path = new PathTrie;  // create a new trie children node
      new_path->character = new_char; //int, in label or index (1 -- 255)
      new_path->timestep = new_timestep;
      new_path->parent = this;
      new_path->log_prob_c = cur_log_prob_c;
      children_.push_back(std::make_pair(new_char, new_path));
                                      ...
                                      const std::vector<std::string>& char_list)
{
  ...
  // byte_is_codepoint_boundary(char_list_[current->character][0])
  if (!byte_is_codepoint_boundary(char_list[character][0])) {
    stop = parent->get_prev_grapheme(output, timesteps, char_list);
  }
  output.push_back(character);
  timesteps.push_back(timestep);
  return stop;
