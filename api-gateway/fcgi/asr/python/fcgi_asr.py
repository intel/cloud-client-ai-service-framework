...
def asr_mic_thread(using_mic, language):
    ...
    if using_mic:
        ...
        wave_file = './asr/wave_file/D4_750_ch.wav' if language == 'CHINESE' else './asr/wave_file/4507-16021-0012_en.wav'
        wf = wave.open(wave_file, 'rb')
        _, _, _, frame_num, _, _ = wf.getparams()
        i = 0
        while not gAsrStop.is_set():
            # simulate mic input with a wave file
            if i + 320 < frame_num :
                wf_frames = wf.readframes(320) #320 frame = 320*2=640 bytes = int(16000*(20ms/1000)*2)= int(sample_rate * (frame_duration_ms / 1000.0) * 2)
                i = i + 320
            elif i == frame_num:
                wf_frames = bytes(320 * 2)
            else:
                wf_frames = wf.readframes(frame_num - i)
    ...
    if not args_realtime:
        ...
        if realtime == 'ONLINE_READ':
            if not gOnlineThreadRunning:
                ...
                p_mic = Thread(target=asr_mic_thread, args=(isUsingMic, language), daemon=True)
