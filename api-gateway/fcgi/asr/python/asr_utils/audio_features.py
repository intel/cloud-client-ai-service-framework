...
class AudioFeaturesSeqPipelineStage(BlockedSeqPipelineStage):
    def __init__(self, profile):
        ...
        window_len = round(sampling_rate * self.p['frame_window_size_seconds']) #在一个窗口(32ms)内有多少个取样点,即有多少帧
        stride_len = round(sampling_rate * self.p['frame_stride_seconds']) #窗口移动多少帧，窗口移动20ms,
