...
class Graph:
    def __init__(self, mode="train"):
        ...
        if mode=="train":
            ...
            print("ie_post")
            self.y = tf.placeholder(tf.float32, shape=(None, None, hp.n_mels * hp.r), name = 'y')
        else: # Synthesize
        ...
        if mode != "ie_post":
           ...
           self.decoder_inputs = self.decoder_inputs[:, :, -hp.n_mels:] # feed last frames only (N, Ty/r, n_mels),取后hp.n_mels个元素，即 [-hp.n_mels:end]
        ...
        with tf.variable_scope("net"):
            if mode=="ie_decode":
               ...
               print("ie_post")
               self.z_hat = decoder2(self.y, is_training=is_training) # (N, T_y//r, (1+n_fft//2)*r)
            else:
               # Encoder
               self.memory = encoder(self.encoder_inputs, is_training=is_training) # (N, T_x, E)
